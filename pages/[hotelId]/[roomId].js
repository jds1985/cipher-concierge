...
export default function MultiHotelRoomChat() {
  const router = useRouter();
  const { hotelId, roomId } = router.query;

  // Format hotel slug for display (e.g., "the-oliver" -> "THE OLIVER")
  const displayHotelName = hotelId
    ? hotelId.replace(/-/g, " ").toUpperCase()
    : "THE HOTEL";

  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: `Welcome. I am Cipher, your private digital concierge for Room ${
        roomId || "Guest Suite"
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I apologize, but I am momentarily experiencing network interference. Please contact the front desk.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <Head>
        <title>{`${displayHotelName} | Room ${roomId || "Guest"} Concierge`}</title>
        <meta
          name="viewport"
      ...
      <div className="chat-viewport">
        <header className="room-header glass-panel">
          <div className="room-brand">
            ...
            />
            <div>
              <h1 className="hotel-name">{displayHotelName}</h1>
        ...
        <footer className="input-dock">
          <form onSubmit={handleSend} className="input-bar glass-panel">
            <input
              ...
              placeholder="Ask about amenities, dining, or area recommendations..."
              disabled={loading}
